...
os.environ.setdefault(
    "DATABASE_URL", "postgresql+asyncpg://assignments:assignments@localhost:5436/assignments_test"
)
...
async def api_client(redis_client, monkeypatch):
    ...
    `with TestClient(...)` scope - the portal runs the app on its own event
    loop (see auth/content/classroom service conftests for the same note)."""
    ...
    # The portal's requests populate app_engine's pool with connections bound
    # to the portal's own loop, which is about to close - dispose again so
    # nothing leftover gets checked out by the next test's loop.
    await app_engine.dispose()
