...
CONSUMER_NAME = "answers-service.content-task-updated"
...
async def _handle_content_event(envelope: EventEnvelope) -> None:
    if envelope.event_type != "task_updated":
        return
    task_id = uuid.UUID(envelope.payload["task_id"])
    client = _event_bus_client()
    try:
        await content_client.invalidate_answer_key(client, task_id=task_id)
    finally:
        await client.aclose()
...
async def run(stop_event: asyncio.Event) -> None:
    ...
    relay_task = asyncio.create_task(
        run_relay_loop(
            r=client,
            config=RelayConfig(producer_name="answers-service"),
    ...
    consumer_task = asyncio.create_task(
        run_consumer_loop(
            ...
            config=ConsumerConfig(
                ...
                group_name="answers-service",
            ),
            handler=_handle_content_event,
